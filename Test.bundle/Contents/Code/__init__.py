...
class TestAgent(Agent.TV_Shows):
    ...
    def search(self, results, media, lang, manual=False):
        ...
        if media.title:
            shootSite = media.title
        else:
            Log("ERROR: No Site for " + media.episode)
        if shootSite:
            results.Append(
                MetadataSearchResult(
                    id=str(shootSite), name=shootSite, year=None, lang=lang, score=100
                )
            )
    ...
    def update(self, metadata, media, lang):
        Log("".ljust(157, "="))
        Log(
            "update() - metadata.id: '%s', metadata.title: '%s'"
